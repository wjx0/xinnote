...
public class RedisCache<K,V> implements Cache<K,V> {
    ...
    /**
     * Constructs a redisManager instance with the specified
     * Redis manager and using a custom key prefix.
     * @param redisManager The redisManager manager instance
    ...
    public V put(K key, V value) throws CacheException {
        logger.info("存储 key [" + key + "]"+"value["+value+"]");
        try {
            redisManager.set(getByteKey(key), SerializationUtils.serialize(value),expire);
            return value;
        } catch (Throwable t) {
            throw new CacheException(t);
        }
